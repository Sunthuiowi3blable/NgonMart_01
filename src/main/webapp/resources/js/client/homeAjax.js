...
function addToCart(id)
	{
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/ngonmart/api/gio-hang/addSanPham?id="+id,
...
$(document).ready(function(){
	...
	function ajaxGet(){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/ngonmart/api/san-pham/latest",
			success: function(result){
				var content;
					...
					if(i != result.length-1)
					{
						if(i%4==0)
						{
							...
							content = '<div class="grid_1_of_4 images_1_of_4 products-info"><a href="sp?id='+sanpham.id+'">' +
									'<img style="width: 300px; height: 238px" src="/ngonmart/img/'+sanpham.id+'.png">' +
